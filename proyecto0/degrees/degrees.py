...
def shortest_path(source, target):
    ...
    while True:
        ...
        if node_remove.state == target:
            ...
            while node_remove.parent is not None:
                action = node_remove.action
        ...
        #Debo agregar los nodos vecionos a la frontera
        for action, state in neighbors_for_person(node_remove.state):
            if not frontier.contains_state(state) and state not in explored:
                new_node = Node(state=state, parent=node_remove, action=action)
